...
import { CheckCircle2, XCircle, CheckCheck, Save } from 'lucide-react'
...
export default function TeacherAttendance() {
  ...
  const markAll = () => setStatuses(Object.fromEntries(students.map(s => [s.id, 'present'])))
            ...
            return (
              <div key={student.id} className="card" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, borderLeft: `4px solid ${status === 'present' ? 'var(--accent-green)' : 'var(--accent-red)'}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Roll No: {student.rollNo}</div>
                </div>
